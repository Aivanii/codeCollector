...
export default function Create() {
    ...
    const [selectedFilters, setSelectedFilters] = useState(["Frontend Development", ""]);
    ...
    const divBlocks = divs.map(elem => {
        if (elem[0].type === "text") {
            const formattedText = elem[0].text.replace(/<c>/g, `<code class = "CodeText">`).replace(/<\/c>/g, '</code>');
            return (
    ...
    function saveCode() {
        ...
        for (let i = 0; i < divs.length; i++) {
            ...
            if (elem.type === "subtitle") {
                ...
                FinalDivs += `<p class="Text">${elem.text}</p>\n`;
            }
        ...
        let TheFinalCode = `<link rel="stylesheet" href="../../src/articlesStyles.css">
            ${FinalDivs}
        `;
            ...
            <Container id="main">
                <TitleContainer>
                    <CategoryContainer>
                        ...
                        <Select name="filter1" onChange={event => setSelectedFilters([event.target.value, ThemesValues[event.target.value][0]])} className="borderer">
                            {SelectBlock1}
                        </Select>
                        <SelectSpan>&gt;</SelectSpan>
                        <Select name="filter2" onChange={event => setSelectedFilters([selectedFilters[0], event.target.value])} className="borderer">
                            {SelectBlock2}
                        </Select>
                        <DateInput type="date" value={dateToday} max={`${date.getFullYear()}-${(date.getMonth() + 1 > 9) ? date.getMonth() + 1 : "0" + date.getMonth()}-${(date.getDay() > 9) ? (date.getDay()) : "0" + date.getDay()}`}
